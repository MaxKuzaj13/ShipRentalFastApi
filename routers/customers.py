...
async def get_starship(customers_id: int = Path(..., title="The ID of the starship to get")):
...
async def create_starship(customer: CustomerSchema):
    new_id = max(customers_db.keys()) + 1
    new_customer = CustomerSchema(id=new_id, name=customer.name, document_number=customer.document_number, address =customer.address)
    customers_db[new_id] = new_customer.dict()
    return customer

@router.put("/{customers_id}", response_model=CustomerSchema)
async def update_starship(customers_id: int, customer: CustomerSchema):
    if customers_id not in customers_db:
        raise HTTPException(status_code=404, detail="Starship not found")
...
async def delete_starship(customers_id: int):
    if customers_id not in customers_db:
