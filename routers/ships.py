...
from schemas.ships import StarshipSchemaRecived, StarshipSchemaStored
...
@router.post("/", response_model=StarshipSchemaRecived, status_code=201)
async def create_starship(starship: StarshipSchemaRecived):
    new_id = max(starships_db.keys()) + 1
    new_starship = StarshipSchemaStored(id=new_id, name=starship.name, available=True, price_per_hour=starship.price_per_hour, max_speed =starship.max_speed, max_range=starship.max_range)
    starships_db[new_id] = new_starship.dict()
    return starship

@router.put("/{starship_id}", response_model=StarshipSchemaStored)
async def update_starship(starship_id: int, starship: StarshipSchemaRecived):
    if starship_id not in starships_db:
        raise HTTPException(status_code=404, detail="Starship not found")
